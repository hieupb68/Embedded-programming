...
#define DHTpin 18  // Set DHT pin as GPIO2
DHTesp dht;
  ...
  if (strcmp(topic, "SMH-001/door_state/MAIN_DOOR") == 0) {
    ...
    if (incomingMessage.equals("Opened")) {
      if (isDoorClosed) {
        ...
        activateBuzzer();
        Serial.println("Cua dang mo hoac da duoc mo boi yeu cau truoc do");
      }
      ...
      if (!isDoorClosed) {
        ...
        activateBuzzer();
        Serial.println("Cua dang dong hoac da duoc dong boi yeu cau truoc do");
      }
    } else {
      Serial.println("Lenh khong hop le cho cua");
    }
  }
}
...
void setup() {
  ...
  lcd.print("Nha thong minh");  //Ghi đoạn text "Welcom to"
